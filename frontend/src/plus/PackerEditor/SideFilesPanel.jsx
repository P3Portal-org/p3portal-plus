...
// p3portal.org
// PROJ-92: Nebendateien-Editor (AC-FILE). Freie Dateien unter files/, die kein
// Provisioner/Installer erzeugt — z. B. der von ssh_private_key_name referenzierte
// SSH-Private-Key. name → inhalt; Pfad-/Traversal-Härtung serverseitig.
  ...
  return (
    <div className="rounded-lg border border-portal-border bg-portal-bg2/40 p-4 space-y-3">
      <div className="flex items-center justify-between gap-2">
        <h4 className="text-sm font-semibold text-portal-text">{t('packer_editor.files.title')}</h4>
      </div>
      <p className="text-[11px] text-portal-text3">{t('packer_editor.files.hint')}</p>

      {names.length === 0 && <p className="text-[11px] text-portal-text3">{t('packer_editor.files.empty')}</p>}
      ...
      <div className="flex gap-1">
        <input
          ...
          value={newName}
          placeholder={t('packer_editor.files.name_ph')}
          onChange={(e) => setNewName(e.target.value)}
          onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); add() } }}
        />
        <button type="button" className="btn-table shrink-0" onClick={add}>+ {t('packer_editor.files.add')}</button>
      </div>
    </div>
  )
}
