...
// p3portal.org
// PROJ-93: Liste editor-verwalteter Playbook-Definitionen (Marker-gefiltert) mit
// Neu / Bearbeiten / Löschen. Fremde ZIP/Git-Playbooks tauchen hier nicht auf.
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between gap-2">
        <p className="text-xs text-portal-text2">{t('ansible_editor.list.intro')}</p>
        <button type="button" className="btn-primary text-xs" onClick={onNew}>
          + {t('ansible_editor.list.new')}
        </button>
      </div>
      ...
      {!isLoading && !error && list.length === 0 && (
        <div className="py-12 text-center">
          <p className="text-sm text-portal-text2">{t('ansible_editor.list.empty')}</p>
        </div>
      )}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-xs text-portal-text2 border-b border-portal-border">
                <th className="px-3 py-2 font-medium">{t('ansible_editor.list.col_name')}</th>
                <th className="px-3 py-2 font-medium">{t('ansible_editor.list.col_id')}</th>
                <th className="px-3 py-2 font-medium">{t('ansible_editor.list.col_target')}</th>
                <th className="px-3 py-2 font-medium">{t('ansible_editor.list.col_tasks')}</th>
                <th className="px-3 py-2 font-medium text-right">{t('ansible_editor.list.col_actions')}</th>
              </tr>
            </thead>
            <tbody>
              {list.map((d) => (
                <tr key={d.id} className="border-b border-portal-border last:border-0">
                  ...
                  <td className="px-3 py-2 text-xs text-portal-text2">
                    {d.targets === 'guest' ? t('ansible_editor.header.target_guest') : t('ansible_editor.header.target_localhost')}
                  </td>
                  <td className="px-3 py-2 text-xs text-portal-text2">{d.task_count}</td>
      ...
      {confirmDel && (
        <ConfirmModal
          title={t('ansible_editor.list.delete_title')}
          body={t('ansible_editor.list.delete_message', { name: confirmDel.name })}
