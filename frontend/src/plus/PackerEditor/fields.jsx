...
// p3portal.org
// PROJ-92: geteilte Formular-Bausteine (Field/ComboField/Toggle), Muster aus
// dem Stacks-Editor (StackResourceCard) übernommen. Pflichtfelder tragen ein
// abschließendes „ *" im Label → design-passende Akzent-Umrandung (theme-Token).
...
export function NumberField({ label, value, onChange, min, max, disabled }) {
  return (
    <Field label={label}>
      <input
        type="number"
 ...
 * keine Optionen vorliegen (API leer/offline) oder der Nutzer „Eigener Wert…"
 * wählt. Identisches Muster wie StackResourceCard.ComboField.
 */
  ...
  if (opts.length === 0) {
    ...
    inner = (
      <div className="flex gap-1">
        ...
        <button
          ...
          title={t('packer_editor.use_list')}
        >☰</button>
      </div>
    )
  } else {
    ...
    inner = (
      ...
      >
        ...
        <option value={MANUAL}>{t('packer_editor.custom_value')}</option>
      </select>
    )
  }
